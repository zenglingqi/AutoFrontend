...
import ProductCard from '../../../../components/shop/ProductCard'; // 确保路径正确

interface Block {
    type: 'heading' | 'text' | 'image_parallax' | 'product_grid' | 'quote' |'accent_paragraph' | 'image_grid' | 'comparison_table';
    data: any;
}
...
export default function BlogContentRenderer({ content, relatedProducts = [] }: ContentRendererProps) {
...
function renderBlock(block: Block, relatedProducts: any[]) {
    switch (block.type) {
        ...
        case 'product_grid':
            // 逻辑处理：如果 block.data 是 ID 数组 ["BRL-001", ...]
            // 我们从 relatedProducts 过滤出对应的完整对象
            const displayProducts = Array.isArray(block.data)
                ? relatedProducts.filter(p => block.data.includes(p.productCode))
                : [];

            if (displayProducts.length === 0) return null;

            return (
                <div className="grid grid-cols-2 md:grid-cols-3 gap-x-4 gap-y-10 my-16">
